...
const DirectorOfficeInbox: React.FC = () => {
  ...
  // Director office specific state
  ...
  const filteredLetters = useMemo(() => {
    ...
    switch (selectedFilter) {
      ...
      case "approve":
        filtered = filtered.filter(
          (letter) =>
            (letter as import("./BaseInbox").Letter).status === "pending"
        );
        break;
      case "reject":
        filtered = filtered.filter(
          (letter) =>
            (letter as import("./BaseInbox").Letter).status === "rejected"
        );
        break;
      case "assigned":
        filtered = filtered.filter(
          (letter) =>
            (letter as import("./BaseInbox").Letter).status === "assigned"
        );
        break;
      default:
        break;
  ...
  // Debug test button to open modal with first letter
  // Place this just before the return statement
  const handleTestOpenModal = () => {
    if (letters.length > 0) {
      setOpenLetter(letters[0]);
      setShowModal(true);
      setViewMode(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] py-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col items-center">
          ...
          <p className="text-sm text-indigo-600 mt-2">
            Director Office Inbox - Multi-unit management
          </p>
          <p className="text-xs text-gray-500 mt-1">
            Managing multiple sub-sub-categories under:{" "}
            {user.departmentOrSector || "Department"}
          </p>
        </div>
              ...
              return (
                ...
                >
                  <span className="flex items-center gap-2">
                    ...
                    >
                      {getFilterCount(visibleLetters, btn.label)}
                    </span>
                  </span>
                </button>
              );
      ...
      {(() => {
        console.log("Modal render check", { openLetter, showModal });
...
export default DirectorOfficeInbox;
