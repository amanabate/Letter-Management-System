...
const CarbonCopyInbox: React.FC = () => {
  ...
  const [viewType, setViewType] = useState<"list" | "card">(
    localStorage.getItem("carbonCopyViewType") === "card" ? "card" : "list"
  );
  const [allUsers, setAllUsers] = useState<any[]>([]);
  const [originalLetterData, setOriginalLetterData] = useState<any | null>(
    null
  );
  ...
  // Only show CC letters
  const ccLetters = useMemo(
    () => letters.filter((l) => l.isCC && l.toEmail === user.email),
    [letters, user.email]
  );

  // Filter buttons for CC inbox
  const ccInboxButtons = [
    { key: "all", label: t.carbonCopy?.filterAll || "All" },
    { key: "unread", label: t.carbonCopy?.filterUnread || "Unread" },
    { key: "starred", label: t.carbonCopy?.filterStarred || "Starred" },
    { key: "urgent", label: t.carbonCopy?.filterUrgent || "Urgent" },
    { key: "seen", label: t.carbonCopy?.filterSeen || "Seen" },
  ];
  ...
  const filteredLetters = useMemo(() => {
    ...
    return filtered;
  }, [ccLetters, selectedFilter, search]);
  ...
  const handleLetterOpen = async (letter: Letter) => {
    ...
    setShowModal(true);
    setViewMode(false);
    setOriginalLetterData(null);
    // Mark as read if unread
    if (letter.unread) {
      updateLetterStatus(letter._id, { unread: false });
    }
    // If this is a CC letter and has an originalLetter, fetch the original letter
    if (letter.isCC && letter.originalLetter) {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/letters/${letter.originalLetter}`
        );
        setOriginalLetterData(res.data);
      } catch (e) {
        setOriginalLetterData(null);
      }
    }
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      ...
      const departmentLabel = openLetter?.department
        ...
        : "";
      // Map CC list to profile names and departments, ensuring no duplicate departments
      const normalize = (str: string) => (str || "").toLowerCase().trim();
      const ccRecipients = ccList.map((cc) => {
        const user = allUsers.find(
          (u) =>
            normalize(u.email) === normalize(cc) ||
            normalize(u.name) === normalize(cc)
        );
        return user
          ? {
              name: user.name,
              department: user.departmentOrSector,
            }
          : { name: cc, department: "" };
      });

      // Always include current user's profile name
      const currentUser = allUsers.find(
        (u) =>
          normalize(u.email) === normalize(user.email) ||
          normalize(u.name) === normalize(user.name)
      );
      if (
        currentUser &&
        !ccRecipients.some(
          (recipient) =>
            normalize(recipient.name) === normalize(currentUser.name)
        )
      ) {
        ccRecipients.push({
          name: currentUser.name,
          department: currentUser.departmentOrSector,
        });
      }

      // Remove duplicates by name and ensure no duplicate departments
      const uniqueRecipients = [];
      const seenNames = new Set();
      const seenDepartments = new Set();

      for (const recipient of ccRecipients) {
        const normName = normalize(recipient.name);
        const normDept = normalize(recipient.department);

        // Skip if we've already seen this name
        if (seenNames.has(normName)) continue;

        // Skip if we've already seen this department (unless it's the current user)
        if (
          normDept &&
          seenDepartments.has(normDept) &&
          normName !== normalize(currentUser?.name || "")
        )
          continue;

        seenNames.add(normName);
        if (normDept) seenDepartments.add(normDept);
        uniqueRecipients.push(recipient);
      }

      const ccNames = uniqueRecipients.map((r) => r.name);
      const ccSection =
        ccNames.length > 0
          ? `<div style='margin-top: 18px;'><div style='font-weight: 700; color: #555; font-size: 16px; margin-bottom: 2px;'><b>CC:</b></div><div style='color: #222; font-size: 14px; margin-left: 10px;'>${ccNames
              .map((cc) => `<div>${cc}</div>`)
              .join("")}</div></div>`
          : "";
      const printContent = `
            ...
            <div class="memo-container">
                ...
                <div class="memo-title-section">
                    <div class="memo-title" style="text-align: center; width: 100%;">
                        <p class="amharic" style="margin: 0 auto;">የውስጥ ማስታወሻ</p>
                ...
                <div class="memo-body">
                    <p><strong>${t.carbonCopy?.from || "From"}:</strong> ${getSenderDisplayName(
                      openLetter
                    )}</p>
                    ${departmentLabel}
                    <p style="text-align: left; margin-left: 25%;"><strong>${t.carbonCopy?.subject || "Subject"}:</strong> ${
                      openLetter.subject
                    }</p>
                    <br>
                ...
                <div class="footer">
                    ...
                    <div class="footer-quote"><p>"ከምድር እስከ ህዋ..."</p></div>
                </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col items-center">
          <h2 className="text-4xl font-extrabold bg-gradient-to-r from-[#b97b2a] via-[#cfae7b] to-[#cfc7b7] text-transparent bg-clip-text drop-shadow-md">
            {t.carbonCopy?.title || "Carbon Copy Inbox"}
          </h2>
          <p className="text-lg text-[#BFBFBF] font-medium">
            {t.carbonCopy?.description ||
              "Your carbon copy letters will appear here."}
          </p>
        </div>
        {/* Controls */}
        <div className="flex flex-col gap-4 mb-8">
          ...
          <div className="flex items-center space-x-3 w-full mb-2">
            <div className="relative w-full sm:w-72">
              <input
                type="text"
                placeholder={
                  t.carbonCopy?.searchPlaceholder || "Search your CC letters..."
                }
                value={search}
            ...
            >
              ...
              {isRefreshing
                ? t.carbonCopy?.refreshing || "Refreshing..."
                : t.carbonCopy?.refresh || "Refresh"}
            </button>
          </div>
          {/* Filter Buttons */}
          <div className="flex gap-2">
            {ccInboxButtons.map((btn) => {
              ...
              return (
                ...
                >
                  <span className="flex items-center gap-2">
                    ...
                    >
                      {getFilterCount(ccLetters, btn.label)}
                    </span>
                  </span>
                </button>
              );
          ...
          <div className="flex justify-end mb-4" key={lang}>
            <button
              ...
              onClick={() => {
                setViewType("list");
                localStorage.setItem("carbonCopyViewType", "list");
              }}
              aria-label="List view"
            >
              <FaThList className="w-4 h-4" /> {t.carbonCopy?.listView || "List"}
            </button>
            <button
              ...
              onClick={() => {
                setViewType("card");
                localStorage.setItem("carbonCopyViewType", "card");
              }}
              aria-label="Card view"
            >
              <FaThLarge className="w-4 h-4" /> {t.carbonCopy?.cardView || "Card"}
            </button>
          </div>
        </div>
        {/* Letters List */}
        <div className="bg-[#FFFFFF] rounded-2xl shadow-xl border border-[#BFBFBF] overflow-hidden">
          {loadingLetters ? (
            <div className="p-12 text-center">
              <LoadingSpinner message="Loading your CC letters..." />
            </div>
            ...
            <div className="p-12 text-center">
              ...
              <p className="mt-4 text-xl text-gray-500">
                {t.carbonCopy?.noLetters || "No carbon copy letters found."}
              </p>
            </div>
          ) : viewType === "list" ? (
        ...
        {totalPages > 1 && (
          <div className="mt-6 flex justify-center">
            <div className="flex space-x-2">
              ...
              >
                {t.carbonCopy?.previous || "Previous"}
              </button>
            ...
            {!viewMode ? (
              <div className="bg-white p-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <div className="bg-gray-100/70 rounded-lg p-3">
                    <div className="text-xs text-gray-500 font-semibold mb-1">
                      {t.carbonCopy?.departmentLabel || "Department"}
                    </div>
                    <div className="text-gray-800 font-semibold text-sm truncate">
                ...
                <div className="mb-4">
                  <div className="text-xs text-gray-500 font-semibold mb-2">
                    {t.carbonCopy?.contentLabel || "Content"}
                  </div>
                ...
                <div className="mb-4">
                  <div className="text-xs text-gray-500 font-semibold mb-2">
                    {t.carbonCopy?.ccLabel || "CC"}
                  </div>
                  <div className="bg-gray-50/50 border-l-4 border-gray-400 rounded-md p-4 text-gray-800 text-sm shadow-inner">
                    {(() => {
                      const normalize = (str: string) =>
                        (str || "").toLowerCase().trim();
                      const ccList = Array.isArray(openLetter.cc)
                        ? openLetter.cc
                        : typeof openLetter.cc === "string" &&
                          openLetter.cc.trim() !== ""
                        ? openLetter.cc
                            .split(",")
                            .map((c) => c.trim())
                            .filter(Boolean)
                        : [];
                      // Map to profile names and departments, ensuring no duplicate departments
                      const ccRecipients = ccList.map((cc) => {
                        const user = allUsers.find(
                          (u) =>
                            normalize(u.email) === normalize(cc) ||
                      ...
                      const ccNames = uniqueRecipients.map((r) => r.name);
                      return ccNames.length > 0 ? ccNames.join(", ") : "None";
                    })()}
                  </div>
                </div>
                  ...
                  openLetter.attachments.length > 0 && (
                    <div className="mt-4">
                      <h4 className="text-md font-semibold text-gray-700 mb-3 flex items-center gap-2">
                        <FaPaperclip />{" "}
                        {t.carbonCopy?.attachments || "Attachments"}
                      </h4>
                          ...
                          >
                            ...
                            <div className="flex space-x-2 items-center shrink-0">
                              ...
                              <button
                                ...
                                title={
                                  t.carbonCopy?.downloadButton || "Download"
                                }
                              >
                                {isDownloadLoading[file.filename] ? (
                  ...
                  recipient={
                    originalLetterData
                      ? (() => {
                          const user = allUsers.find(
                            (u) =>
                              u.email === originalLetterData.toEmail ||
                              u.name === originalLetterData.toName
                          );
                          return (
                            user?.name ||
                            originalLetterData.toName ||
                            originalLetterData.toEmail ||
                            ""
                          );
                        })()
                      : getRecipientProfileName(openLetter)
                  ...
                  recipientDepartment={
                    originalLetterData
                      ...
                      : (() => {
                          const user = allUsers.find(
                            (u) =>
                              u.email === openLetter.toEmail ||
                              u.name === openLetter.toName
                          );
                          return (
                            user?.departmentOrSector || openLetter.department
                          );
                        })()
                  }
                  reference={""}
      ...
      >
        <div className="w-full h-full flex flex-col">
            ...
            {previewType.startsWith("image/") ? (
              ...
              <div className="text-center p-8">
                ...
                <p className="text-xl text-gray-600 mb-2">
                  {t.carbonCopy?.previewNotAvailable ||
                    "Preview not available."}
                </p>
                <p className="text-sm text-gray-500">
                  {t.carbonCopy?.downloadToView ||
          ...
          <div className="p-4 bg-white border-t border-gray-200 flex justify-end space-x-3">
            ...
            >
              {t.carbonCopy?.closeButton || "Close"}
            </button>
            <button
              onClick={() => {
                if (previewUrl) {
...
export default CarbonCopyInbox;
