...
const UserInbox: React.FC = () => {
  ...
  // User-specific inbox buttons (limited functionality)
  const inboxButtons = [
    ...
    { key: "assigned", label: t.inbox?.filterAssigned || "Assigned" }, // <-- new filter
  ];
  ...
  const filteredLetters = useMemo(() => {
    let filtered = letters.filter((letter) => !(letter.isCC === true) && !letter.archived); // Exclude CC and archived letters
  ...
  useEffect(() => {
    // Fetch departments for forward modal
    axios
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      ...
      const departmentLabel = openLetter.department
        ? `<div style='color: #888; font-size: 13px; margin-left: 10px; margin-top: 2px;'>${openLetter.department
            .split(">")
            .pop()
            .trim()}</div>`
        : "";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col items-center">
          ...
          <p className="text-sm text-blue-600 mt-2">
            Regular User Inbox - Limited functionality
          </p>
        </div>
            ...
            <div className="flex flex-col gap-y-4 divide-y divide-gray-100">
              {currentLetters.map((letter) => (
                <LetterListItem
                  ...
                  user={user}
                  t={t}
                  getRecipientProfileName={getRecipientProfileName}
                />
              ))}
            </div>
          ) : (
            ...
            {!viewMode ? (
              <div className="bg-white p-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <div className="bg-gray-100/70 rounded-lg p-3">
                    <div className="text-xs text-gray-500 font-semibold mb-1">
                      {t.inbox && (t.inbox as any).toLabel
                        ? (t.inbox as any).toLabel
                        : "To"}
                    </div>
                    <div className="text-gray-800 font-semibold text-sm truncate">
              ...
              <div className="p-4">
                <TemplateMemoLetter
                  ...
                  recipientDepartment={openLetter.department}
                  reference={""}
                  body={openLetter.content}
                  signature={openLetter.fromName}
                  cc={
                    Array.isArray(openLetter.cc)
                      ? openLetter.cc.join(", ")
                      : openLetter.cc || ""
                  }
                />
