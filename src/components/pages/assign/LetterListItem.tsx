...
import { StarIcon, ArchiveIcon } from "lucide-react";
import {
  Letter,
  getPriorityBadge,
...
const LetterListItem: React.FC<LetterListItemProps> = ({
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 p-6 border border-gray-200 cursor-pointer mx-4 my-4 ${
        letter.unread ? "ring-2 ring-blue-400" : ""
      } ${isActive ? "bg-blue-50" : ""}`}
      onClick={() => {
        console.log("LetterListItem clicked", letter);
        onOpen(letter);
      }}
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3 flex-1">
          <h3 className="font-bold text-lg text-[#003F5D] hover:text-[#C88B3D] transition-colors duration-300 truncate">
            {letter.subject}
          </h3>
      ...
      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mb-2">
        ...
        <span>
          ...
          {getRecipientProfileName
            ...
            : getRecipientDisplayName(letter)}
        </span>
        <span>
        ...
        {letter.attachments && letter.attachments.length > 0 && (
          <span className="flex items-center gap-1 text-[#D29341]">
            📎 {letter.attachments.length} attachment(s)
          </span>
        )}
      </div>
