...
const TopLevelSent: React.FC = () => {
  ...
  // Filter letters for top-level executives (all letters across the organization)
  const filterLetters = (allLetters: Letter[]) => {
    return allLetters.filter((letter) => {
      ...
      // Include letters sent by the top-level executive themselves
      if (letter.fromEmail === userEmail) {
        return true;
      }
      // Include all letters from the organization
      // Top-level executives can see all sent letters
      return true;
    });
  };
  ...
  return (
    <BaseSent
      title={t.sent?.organizationSentLetters || "Organization Sent Letters"}
      subtitle={t.sent?.organizationSentLettersSubtitle || "Overview of all sent letters across the organization"}
        ...
        {filteredLetters.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">
              No letters have been sent across the organization yet.
            </p>
          </div>
        )}
      </div>
