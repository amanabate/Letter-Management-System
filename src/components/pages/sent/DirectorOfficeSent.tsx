...
const DirectorOfficeSent: React.FC = () => {
  ...
  // Filter letters for director office (letters from multiple sub-sub-categories they oversee)
  const filterLetters = (allLetters: Letter[]) => {
    return allLetters.filter((letter) => {
      ...
      // Include letters sent by the director office user themselves
      if (letter.fromEmail === userEmail) {
        return true;
      }
      // Include letters from sub-sub-categories they oversee
      // Director office typically oversees multiple sub-sub-categories
      const letterDepartment = letter.department || "";
      // Check if the letter is from any sub-sub-category under their jurisdiction
      // This is a simplified check - in a real system, you'd have a more complex hierarchy
      return letterDepartment.includes(userDepartment) || 
             userDepartment.includes(letterDepartment) ||
             // Add logic for multiple sub-sub-categories if needed
             letterDepartment.startsWith(userDepartment.split('/')[0]); // Check main category
    });
  };
  ...
  return (
    <BaseSent
      title={t.sent?.directorOfficeSentLetters || "Director Office Sent Letters"}
      subtitle={`${t.sent?.directorOfficeSentLettersSubtitle || "Manage sent letters from multiple sub-sub-categories under"} ${userDepartment}`}
        ...
        {filteredLetters.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">
              No letters have been sent from the sub-sub-categories you oversee yet.
            </p>
          </div>
        )}
      </div>
...
export default DirectorOfficeSent; 